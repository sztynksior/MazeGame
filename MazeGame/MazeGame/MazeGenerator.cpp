...
MazeCell* MazeGenerator::getAccessToCellIndicatedByVector(const std::pair<int, int> pCellCoordinates, const  std::pair<int, int> pVector)
{
...
MazeGenerator::MazeGenerator(const int pMaxMazeX, const int pMaxMazeY, const std::pair<int, int>& pStartingCoordinates)
{
	...
	for (int i = 0; i < this->maxMazeX; i++)
	{
		...
		{
			this->maze[i].push_back(new MazeCell({ i, j }));
		}
	}
...
int MazeGenerator::getMaxMazeX()
{
	return this->maxMazeX;
}

int MazeGenerator::getMaxMazeY()
{
	return this->maxMazeY;
}

MazeCell* MazeGenerator::getAccessToCellWithGivenCoordinates(std::pair<int, int> pCoordinates)
{
	return maze[pCoordinates.first][pCoordinates.second];
}

void MazeGenerator::mazeInicialization()
{
	...
	{
		std::vector<fromWhichSide> notVisitedNeighbours;
